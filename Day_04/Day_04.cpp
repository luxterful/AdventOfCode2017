...
string sorting(string word){
    for(int idx_x = 0; idx_x < word.length(); idx_x++){
        int min_idx = idx_x;
        for(int idx_y = idx_x; idx_y < word.length(); idx_y++){
            if(word[idx_x] < word[min_idx]) min_idx = idx_x;
        }
        char temp = word[idx_x];
        word[idx_x] = word[min_idx];
        word[min_idx] = temp;
    }
    return word;
}

bool is_anagram(string word, string compare){
    if(word.length() == compare.length()){
        std::sort(word.begin(), word.end());
        std::sort(compare.begin(), compare.end());
        for(int idx_x = 0; idx_x < word.length(); idx_x++){
            if(word[idx_x] != compare[idx_x]) return false;
        }
        return true;
    }
    return false;
...
int part_I(string input){
    int valid_sum = 0;

    istringstream doc_stream(input);

    string line;
    while(getline(doc_stream, line)){
        vector<string> words = split(line, ' ');

        bool line_valid = true;
        for(int idx_x = 0; idx_x < words.size(); idx_x++){
            for(int idx_y = 0; idx_y < words.size(); idx_y++){
                if(words[idx_x] == words[idx_y] && idx_x!=idx_y){
                    line_valid = false;
                }
            }
        }

        if(line_valid) valid_sum++;
    }
...
int part_II(string input){
    ...
    while(getline(doc_stream, line)){
        ...
        for(int idx_x = 0; idx_x < words.size(); idx_x++){
            for(int idx_y = 0; idx_y < words.size(); idx_y++){
                if(is_anagram(words[idx_x], words[idx_y]) && idx_x!=idx_y){
                    line_valid = false;
                }
            }
        }
